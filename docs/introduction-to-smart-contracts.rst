...
このようなことはする必要はありません。コンパイラが代わりにやってくれます。
...
``public`` キーワードで作成した :ref:`ゲッター関数<getter-functions>` は、マッピングの場合は複雑です。次のようになります。
...
:ref:`require <assert-and-require>` 関数の呼び出しでは、条件を定義し、満たされない場合はすべての変更をリバートします。
...
**ガスプライス** はトランザクションのオリジネーターが設定する値であり、オリジネーターはEVM実行者に ``gas_price * gas`` を前払いする必要があります。
実行後にガスが残っている場合、それはトランザクションのオリジネーターに返金されます。
