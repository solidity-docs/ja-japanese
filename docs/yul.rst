...
``F`` 、 ``H`` 、 ``B`` 、 ``C`` 、 ``I`` 、 ``L`` 、 ``P`` 、 ``N`` とマークされたオペコードは、それぞれFrontier、Homestead、Byzantium、Constantinople、Istanbul、London、Parisから存在しています。

以下において、 ``mem[a...b)`` はメモリ上の位置 ``a`` から ``b`` 未満までのバイト列を表し、 ``storage[p]`` はスロット ``p`` におけるストレージの内容を、同様に ``transientStorage[p]`` はスロット ``p`` におけるトランジェントストレージの内容を表します。
...
最適化の手順に関する詳細情報や略語の一覧は、:ref:`オプティマイザのドキュメント <optimizer-steps>` に記載されています。
